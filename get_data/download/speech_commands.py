...
DATA_DIR = "../data/raw/SpeechCommands/SpeechCommands"
...
def download():
    ...
    directory = DATA_DIR + "/speech_commands"
    mkdir_if_not_exists(directory)

    loc = directory + "/speech_commands_data.tar.gz"
    extract_loc = DATA_DIR + "/speech_commands/raw_speech_commands_data"
    if os.path.exists(extract_loc):
        print(
            "Path already exists at {}. If you wish to re-download you must delete this folder.".format(
                extract_loc
            )
        )
        return
...
def process_data():
    base_loc = "../data/raw/SpeechCommands/speech_commands/raw_speech_commands_data"
    X = torch.empty(34975, 16000, 1)
    y = torch.empty(34975, dtype=torch.long)

    batch_index = 0
    y_index = 0
    for foldername in tqdm(
    ...
    X = (
        torchaudio.transforms.MFCC(
            log_mels=True, melkwargs=dict(n_fft=360, n_mels=32), n_mfcc=10
        )(X.squeeze(-1))
        .transpose(1, 2)
        .detach()
    )
...
def get_data():
    loc = DATA_DIR + "/speech_commands/processed_speech_commands_data"
    try:
...
if __name__ == "__main__":
    ...
    train_data, test_data = (
        torch.cat([x.dataset.tensors[0] for x in train_dls]),
        test_dl.dataset.tensors[0],
    )
    train_labels, test_labels = (
        torch.cat([x.dataset.tensors[1] for x in train_dls]),
        test_dl.dataset.tensors[1],
    )
    num_train = train_data.size(0)
    num_test = test_data.size(0)
    original_idxs = [
        list(range(0, num_train)),
        list(range(num_train, num_train + num_test)),
    ]
    ...
    np.savez(DATA_DIR + "/data_len=89.npz", data=data.numpy(), labels=labels.numpy())
