...
from helpers import mkdir_if_not_exists, download_url, unzip

DATA_DIR = "../data"
RAW_DIR = "../data/raw/TSR"
PROCESSED_DIR = "../data/raw/TSR"
...
def download():
    ...
    assert os.path.isdir(
        raw_dir
    ), "No directory exists at data/raw. Please make one to continue."
...
def convert_data():
    ...
    for dataset in tqdm(datasets):
        ...
        if os.path.exists(save_loc):
            print(
                "Skipping {} as folder exists at {}. Remove it to reconvert.".format(
                    dataset, save_loc
                )
            )
            # continue
        ...
        try:
            ...
            X_train, y_train = load_from_tsfile_to_dataframe(
                loc + "/" + dataset + "_TRAIN.ts"
            )
            X_test, y_test = load_from_tsfile_to_dataframe(
                loc + "/" + dataset + "_TEST.ts"
            )
            ...
            for idx in range(all_frames.shape[0]):
                tensor_data.append(
                    torch.Tensor(pd.concat(all_frames.iloc[idx].values, axis=1).values)
                )

            # Keep track of the original train test split idxs
            num_train, num_test = X_train.shape[0], X_test.shape[0]
            original_idxs = torch.arange(0, num_train), torch.arange(
                num_train, num_test + num_train
            )
            ...
            try:
                ...
                raise Exception(
                    "Could not stack the data for dataset: {}".format(dataset)
                )
